...
class LoudnessNormalizer(BaseAgent):
    def __init__(self):
        super().__init__()
    ...
    def process_message(self, message):
        ...
        # 构建命令，使用完整路径
        cmd = [self.fap_path, "loudness-norm", str(abs_audio_dir), str(abs_audio_dir), "--overwrite"]
        cmd_str = " ".join(cmd)
        ...
        try:
            ...
            print(f"音量标准化完成，返回码: {return_code}")

            # 返回处理结果
            return Message(
                content={
                    "status": "success" if return_code == 0 else "error",
                    "message": "Audio Normalization completed successfully" if return_code == 0
                    else f"Audio Normalization failed with return code {return_code}"
                }
            )

        except Exception as e:
            print(f"音量标准化出错: {str(e)}")
            return Message(
                content={
                    "status": "error",
                    "message": f"Audio Normalization failed: {str(e)}"
                }
            )

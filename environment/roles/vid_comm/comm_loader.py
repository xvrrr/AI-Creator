...
async def content_main(config=None):
    ...
    # Default paths
    txt_path = os.path.join(writing_data_dir, "source_text.txt")
    pre_txt_path = os.path.join(writing_data_dir, "present_style.txt")
    
    # Override txt_path if source_text is provided in config
    if config and 'source_text' in config and config['source_text']:
        source_text = config['source_text']
        if os.path.exists(source_text):
            print(f"Using custom source text file: {source_text}")
            txt_path = source_text
        else:
            print(f"Warning: Custom source text file not found: {source_text}")
    
    # Get idea from config or prompt user
    user_idea = None
    if config and 'idea' in config:
        ...
        print("\n=== CREATING CONTENT ===")
        user_idea = input("\nPlease describe your commentary video idea, and please indicate your word count requirement (1500 字/3300 words about 3 minutes): ")

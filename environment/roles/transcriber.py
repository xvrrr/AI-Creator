...
class Transcriber(BaseAgent):
    ...
    def process_message(self, message):
        ...
        # 构建命令，使用完整路径
        cmd = [self.fap_path, "transcribe", "--model-type", "funasr", str(abs_audio_dir)]
        cmd_str = " ".join(cmd)
        ...
        try:
            ...
            print(f"转录完成，返回码: {return_code}")

            # 返回处理结果
            return Message(
                content={
                    "status": "success" if return_code == 0 else "error",
                    "message": "Audio transcribe completed successfully" if return_code == 0
                    else f"Audio transcribe failed with return code {return_code}"
                }
            )
            ...
            return Message(
                content={
                    "status": "error",
                    "message": f"Audio transcribe failed: {str(e)}"
                }
            )